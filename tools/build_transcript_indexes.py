...
def season_page(season: int, episodes: list[tuple[str, str, str, str]]) -> str:
    ...
    for number,(slug,title,code,description) in enumerate(episodes,1):
        cards.append(f'''<a class="episode-card" href="/transcripts/season-{season}/{slug}/"><img src="/assets/transcript-stills/{slug}.webp" alt="Scene from {title}" loading="lazy"><div class="episode-copy"><span>File {number:02d} · {code}</span><h2>{title}</h2><p>{description}</p><b>Read transcript →</b></div></a>''')
